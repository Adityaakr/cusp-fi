...
export const DFLOW_MARKETS_MAX_PAGE_LIMIT = 255;
export const DFLOW_SCOPED_MARKETS_LIMIT = 200;

const DEFAULT_EVENTS_PAGE_LIMIT = 100;
const DEFAULT_MARKETS_PAGE_LIMIT = 200;

function normalizePageLimit(limit: number, max: number): number {
...
export interface DFlowMarketCategoryIndex {
  markets: CuspMarket[];
  marketsByTicker: Record<string, CuspMarket>;
  tagsByCategoryLabel: Record<string, string[]>;
  countsByCategory: Record<string, number>;
  countsBySubCategory: Record<string, Record<string, number>>;
  marketsByCategory: Record<string, CuspMarket[]>;
  marketsBySubCategory: Record<string, Record<string, CuspMarket[]>>;
}
...
export async function fetchMarkets(params?: {
  ...
  if (params?.limit) {
    search.set(
      "limit",
      String(normalizePageLimit(params.limit, DFLOW_MARKETS_MAX_PAGE_LIMIT))
    );
  }
...
async function fetchActiveMarketsForSeriesTickers(params: {
  seriesTickers: string[];
  ...
  const limit = normalizePageLimit(
    params.limit ?? DFLOW_SCOPED_MARKETS_LIMIT,
    DFLOW_SCOPED_MARKETS_LIMIT
  );
  const seen = new Set<string>();
...
export async function fetchScopedMarkets(params: {
  ...
  const sourceTag = params.tag?.trim() || undefined;
  const limit = normalizePageLimit(
    params.limit ?? DFLOW_SCOPED_MARKETS_LIMIT,
    DFLOW_SCOPED_MARKETS_LIMIT
  );
  ...
  return fetchActiveMarketsForSeriesTickers({
    seriesTickers,
    categoryLabel,
    sourceTag,
    limit,
...
/**
 * Build a DFlow-native, frontend-local category index.
 *
 * Parent categories come from `/series?category=...`; subcategories come from
 * `/series?category=...&tags=...`. Markets are then fetched through nested
 * events, so category children are based on DFlow taxonomy rather than ticker
 * heuristics or the currently loaded market page.
 */
export async function fetchMarketCategoryIndex(
  tags: DFlowTagsResponse
): Promise<DFlowMarketCategoryIndex> {
  const tagsByCategoryLabel: Record<string, string[]> = {};
  const countsByCategory: Record<string, number> = {};
  const countsBySubCategory: Record<string, Record<string, number>> = {};
  const marketsByCategory: Record<string, CuspMarket[]> = {};
  const marketsBySubCategory: Record<string, Record<string, CuspMarket[]>> = {};
  const categorySeen: Record<string, Set<string>> = {};
  const allSeen = new Set<string>();
  const marketsByTicker: Record<string, CuspMarket> = {};
  ...
  const activeMarkets = await fetchAllActiveMarkets();
  for (const rawMarket of activeMarkets) {
    const market = dflowMarketToCusp(rawMarket);
    const category = resolveMarketCategory(market.eventTicker, market.name, tags);
    const indexedMarket = { ...market, category };
    if (!marketsByCategory[category]) marketsByCategory[category] = [];
    if (!categorySeen[category]) categorySeen[category] = new Set<string>();
    addUniqueMarket(marketsByCategory[category], categorySeen[category], indexedMarket);
    if (!marketsByTicker[indexedMarket.ticker]) marketsByTicker[indexedMarket.ticker] = indexedMarket;
    addUniqueMarket(allMarkets, allSeen, indexedMarket);
  }

  for (const [catKey, tagList] of entries) {
    const label = toTitleCaseCategory(catKey);
    const rawList = tagList!
      .filter((t): t is string => typeof t === "string" && t.trim().length > 0)
      .map((t) => t.trim());

    tagsByCategoryLabel[label] = rawList;
    countsBySubCategory[label] = {};
    marketsBySubCategory[label] = {};
    if (!marketsByCategory[label]) marketsByCategory[label] = [];
    if (!categorySeen[label]) categorySeen[label] = new Set<string>();

    const childResults = await Promise.all(
      rawList.map(async (tag) => {
        const tagSeries = await fetchSeries({ category: label, tags: tag });
        const tagSeriesTickers = (tagSeries.series ?? [])
          .map((series) => series.ticker)
          .filter(Boolean);
        const tagMarkets = await fetchActiveMarketsForSeriesTickers({
          seriesTickers: tagSeriesTickers,
          categoryLabel: label,
          sourceTag: tag,
        });
        return { tag, markets: tagMarkets };
      })
    );

    for (const { tag, markets } of childResults) {
      const childSeen = new Set<string>();
      const childMarkets: CuspMarket[] = [];
      for (const market of markets) {
        addUniqueMarket(childMarkets, childSeen, market);
        addUniqueMarket(marketsByCategory[label], categorySeen[label], {
          ...market,
          subCategory: undefined,
          sourceTag: undefined,
        });
        if (!marketsByTicker[market.ticker]) marketsByTicker[market.ticker] = market;
        addUniqueMarket(allMarkets, allSeen, market);
      }
      marketsBySubCategory[label][tag] = childMarkets;
      countsBySubCategory[label][tag] = childMarkets.length;
    }
  }

  for (const [label, markets] of Object.entries(marketsByCategory)) {
    countsByCategory[label] = markets.length;
  }
...
export async function fetchTotalActiveMarketsCount(options?: { pageLimit?: number }): Promise<number> {
  const limit = normalizePageLimit(
    options?.pageLimit ?? DEFAULT_MARKETS_PAGE_LIMIT,
    DFLOW_MARKETS_MAX_PAGE_LIMIT
  );
  let total = 0;
...
export async function fetchAllActiveMarkets(options?: { pageLimit?: number }): Promise<DFlowMarket[]> {
  const limit = normalizePageLimit(
    options?.pageLimit ?? DEFAULT_MARKETS_PAGE_LIMIT,
    DFLOW_MARKETS_MAX_PAGE_LIMIT
  );
  const all: DFlowMarket[] = [];
  let cursor: number | undefined = undefined;
  for (let i = 0; i < 500; i++) {
    const res = await fetchMarkets({ status: "active", limit, cursor });
