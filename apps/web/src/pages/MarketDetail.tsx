...
const MarketDetail = () => {
  ...
  const buildDirectTradeError = useCallback(
    async (rawMessage: string, walletAddress: string, userMainnetUsdc: number, amountUsdc: number) => {
      const normalized = rawMessage.toLowerCase();
      let mainnetSol = 0;

      try {
        mainnetSol = (await getMainnetConnection().getBalance(new PublicKey(walletAddress), "confirmed")) / 1e9;
      } catch {
        mainnetSol = 0;
      }

      const hasEnoughUsdc = userMainnetUsdc + 1e-9 >= amountUsdc;
      const hasEnoughSolForFees = mainnetSol > 0.002;
      ...
      if (
        ...
        if (!hasEnoughUsdc) {
          return `Trade failed because the wallet does not have enough mainnet USDC. Have $${userMainnetUsdc.toFixed(2)}, need $${amountUsdc.toFixed(2)}.`;
        }
        return `Trade failed because the wallet does not have enough mainnet SOL for fees. Mainnet SOL balance is ${mainnetSol.toFixed(4)}.`;
      }

      if (normalized.includes("simulation")) {
        const hints: string[] = [];
        if (!hasEnoughUsdc) {
          hints.push(`wallet USDC is $${userMainnetUsdc.toFixed(2)} but the trade needs $${amountUsdc.toFixed(2)}`);
        }
        if (!hasEnoughSolForFees) {
          hints.push(`mainnet SOL for fees is low (${mainnetSol.toFixed(4)} SOL)`);
        }

        if (hints.length === 0) {
          return `Transaction simulation failed on the DFlow route. Your wallet has enough displayed USDC, so this is more likely a route/account issue than a simple balance issue. Details: ${rawMessage}`;
        }

        return `Transaction simulation failed. Possible causes: ${hints.join(" · ")}. Details: ${rawMessage}`;
      }
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          throw new Error(
            await buildDirectTradeError(msg, solanaAddress, userMainnetUsdc, amountNum)
          );
        }
        throw signErr;
      }
