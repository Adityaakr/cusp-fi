...
export const faqItems = [
  { q: "Is my USDC safe in the vault?", a: "The vault deploys capital exclusively into high-probability outcome tokens (>85% implied probability) and lending markets. All positions are diversified across multiple markets with strict risk limits. However, prediction market outcomes carry inherent binary risk — the vault manages this through position sizing and probability thresholds." },
  { q: "What happens if a market resolves unexpectedly?", a: "If a market resolves against the vault's position, the loss is absorbed by the vault's diversified portfolio. Individual depositor impact is limited by per-market exposure caps (max 8% of TVL per market). The vault's historical performance accounts for occasional losses." },
  { q: "How does liquidation work on Cusp Lend?", a: "Liquidation triggers when your health factor drops below 1.0, meaning your collateral value no longer sufficiently covers your loan. Additionally, all loans are subject to hard expiry — positions are automatically liquidated 2 hours before market resolution regardless of health factor. This protects the protocol from binary settlement risk." },
  { q: "What is DFlow?", a: "DFlow tokenizes prediction market outcomes from Kalshi as SPL tokens on Solana. Each market has a YES token (yesMint) and NO token (noMint). These are real on-chain assets that can be traded, used as collateral, and composed with other DeFi protocols — which is exactly what Cusp does." },
  { q: "Is Kalshi regulated?", a: "Yes. Kalshi is a CFTC-regulated exchange — the first federally regulated prediction market in the United States. This means the underlying markets Cusp operates on have regulatory clarity, unlike most crypto prediction platforms." },
  { q: "How is the 15–25% APY generated?", a: "Three sources: (1) Position farming — buying YES tokens priced >85¢ that resolve to $1, capturing the spread, (2) Lending spread — earning interest from borrowers using Cusp Lend, (3) LP fees — providing liquidity in secondary markets. The combined yield varies based on market conditions and available opportunities." },
];
