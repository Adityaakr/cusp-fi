...
│                                                     │
│   ┌───────────┐  ┌───────────┐  ┌───────────────┐  │
│   │  Lending   │  │  Margin   │  │  Collateral   │  │
│   │  Pool      │  │  System   │  │  Custody      │  │
│   │  (Vault)   │  │ (Leverage)│  │  (Escrow)     │  │
│   └─────┬─────┘  └─────┬─────┘  └───────┬───────┘  │
│         │              │                │           │
│         └──────────────┼────────────────┘           │
│                        │                            │
...
┌────────────────────────▼────────────────────────────┐
│              MARKET LAYER (DFlow)                   │
│     Kalshi contracts → SPL outcome tokens           │
└────────────────────────┬────────────────────────────┘
                         │
┌────────────────────────▼────────────────────────────┐
│            SETTLEMENT LAYER (Kalshi)                │
│      CFTC-regulated · real-world event contracts    │
└─────────────────────────────────────────────────────┘
```
...
```
User: "I want 3× on BTC > $150K YES with $100"

  ① Risk check        → CUSP API validates market, leverage, liquidity
  ② LP pool lends      → $200 USDC transferred to user wallet (server-signed)
  ③ DFlow order         → $300 quote for YES tokens
  ④ User signs          → Phantom/Solflare signs the mainnet transaction
  ⑤ Execution           → Trade settles on Solana via DFlow
  ⑥ Position tracked    → $100 margin · $200 borrowed · HF monitored

Result: 3× exposure, fully collateralized, liquidation-protected
```
...
└──────────┬──────────────────────────────┬────────────────────┘
           │                              │
  ┌────────▼──────────┐         ┌────────▼─────────────────┐
  │  React Frontend   │         │  Express.js API Server   │
  │  Vite · Tailwind  │◄───────►│  DFlow proxy · WS relay  │
  │  shadcn/ui        │         │  Leverage trade engine    │
  │  Framer Motion    │         │  Risk checks · Lending   │
  │  Recharts         │         └────────┬─────────────────┘
  └────────┬──────────┘                  │
           │                    ┌────────▼─────────────────┐
  ┌────────▼──────────┐        │  External Integrations   │
  │    Supabase       │        │  DFlow · Kalshi · Kamino  │
  │  Positions · Txs  │        │  Jupiter · QuickNode      │
  │  Rate history     │        └───────────────────────────┘
  └───────────────────┘
                         ┌──────────────────────────────────┐
                         │   Solana Blockchain              │
                         │   4 Native Rust Programs         │
                         │                                  │
                         │   ┌────────────┐ ┌────────────┐  │
                         │   │ Earn Vault │ │  Leverage  │  │
                         │   │ USDC→cUSDT │ │ 1x – 5x   │  │
